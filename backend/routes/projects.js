...
router.put('/:id', authenticateToken, (req, res) => {
  ...
  db.prepare('UPDATE projects SET name = ?, description = ? WHERE id = ?').run(name.trim(), description || null, projectId);
...
router.post('/:id/members', authenticateToken, (req, res) => {
  const projectId = parseInt(req.params.id);
  if (!isProjectAdmin(projectId, req.user.id)) return res.status(403).json({ error: 'Admin access required' });
  ...
  if (isProjectMember(projectId, user.id)) return res.status(409).json({ error: 'User is already a member' });
...
router.put('/:id/members/:userId', authenticateToken, (req, res) => {
  ...
  if (targetUserId === req.user.id && role === 'member') {
    const admins = db.prepare("SELECT COUNT(*) as count FROM project_members WHERE project_id = ? AND role = 'admin'").get(projectId);
    if (admins.count <= 1) return res.status(400).json({ error: 'Cannot demote the only admin' });
  }
...
router.delete('/:id/members/:userId', authenticateToken, (req, res) => {
  ...
  const targetUserId = parseInt(req.params.userId);
  if (!isProjectAdmin(projectId, req.user.id)) return res.status(403).json({ error: 'Admin access required' });
  if (targetUserId === req.user.id) return res.status(400).json({ error: 'Cannot remove yourself' });
